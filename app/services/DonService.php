...
class DonService
{
  ...
  public function dispatchDons()
  {
    ...
    try {
      ...
      // 2. Récupérer les dons non utilisés (triés FIFO par date de saisie)
      $dons = $this->donRepository->getDonsNonUtilises();

      // 3. Grouper les dons par besoin pour un accès rapide
      $donsByBesoin = [];
      foreach ($dons as $don) {
        $besoinId = $don['id_besoin'];
        if (!isset($donsByBesoin[$besoinId])) {
          $donsByBesoin[$besoinId] = [];
        }
        $donsByBesoin[$besoinId][] = $don;
      }
      ...
      foreach ($besoins as $besoin) {
        ...
        // Vérifier s'il existe des dons pour ce besoin
        if (!isset($donsByBesoin[$besoinId]) || empty($donsByBesoin[$besoinId])) {
          continue;
        }
        ...
        foreach ($donsByBesoin[$besoinId] as &$don) {
