  ...
  try {
    ...
    if (!empty($stats['details'])) {
      ...
      foreach ($stats['details'] as $i => $detail) {
        echo sprintf(
          "  %d. Don #%d (%s, %s) → Besoin #%d (%s - %s, %s) : %d unités\n",
          $i + 1,
          $detail['don_id'],
          $detail['type'],
          $detail['don_date'],
          $detail['besoin_id'],
          $detail['besoin_libelle'],
          $detail['type'],
          $detail['ville'],
          $detail['quantity']
        );
  ...
  try {
    ...
    if (!empty($report['besoins_non_satisfaits']['details'])) {
      ...
      foreach ($besoins as $besoin) {
        echo sprintf(
          "  #%-3d %-30s | %-15s | %-15s | %4d unités | %.2f Ar\n",
          $besoin['id'],
          $besoin['libelle'],
          $besoin['type_libelle'],
          $besoin['ville_nom'],
...
$dispatchAdd
  ...
  ->action(function ($besoin, $quantity, $date) {
    ...
    try {
      ...
      // Si aucun paramètre, afficher les besoins disponibles
      if (!$besoin || !$quantity) {
        echo "❌ Erreur: ID du besoin et quantité sont obligatoires\n\n";
        echo "📋 Besoins disponibles:\n";
        echo "────────────────────\n";

        $besoins = $donRepository->getBesoinsDisponibles();
        if (empty($besoins)) {
          echo "  Aucun besoin disponible actuellement\n";
        } else {
          foreach ($besoins as $b) {
            echo sprintf(
              "  #%-3d %-30s | %-12s | %-15s | %4d unités\n",
              $b['id'],
              $b['libelle'],
              $b['type_libelle'],
              $b['ville_nom'],
        ...
        echo "\n💡 Utilisation:\n";
        echo "  php vendor/bin/runway dispatch:add -b <ID_BESOIN> -q <QUANTITE>\n";
        echo "\nExemple:\n";
        echo "  php vendor/bin/runway dispatch:add -b 1 -q 100\n\n";
        return 1;
      }
      ...
      // Créer le don
      $donId = $donService->createDon($besoin, $quantity, $date);
