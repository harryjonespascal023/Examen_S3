...
if (!empty($besoin)) {
  ...
  $prixUnitaireValue = (float) ($besoin->prix_unitaire ?? 0);
...
<div class="page-header">
  <div class="d-flex align-items-center justify-content-between">
    <h1><i class="bi bi-exclamation-triangle-fill text-warning"></i>
      <?php echo htmlspecialchars($title, ENT_QUOTES, 'UTF-8'); ?></h1>
    <a class="btn btn-secondary" href="<?= BASE_URL ?>/besoins">
    ...
    <form method="post" action="<?php echo htmlspecialchars($action, ENT_QUOTES, 'UTF-8'); ?>">
      <div class="row">
        ...
        <div class="col-md-6 mb-3">
          <label class="form-label" for="id_type_besoin">
            <i class="bi bi-tag"></i> Type de besoin (catégorie) *
          </label>
          <select class="form-select" id="id_type_besoin" name="id_type_besoin" required>
            <option value="">Choisir un type</option>
            <?php foreach (($types ?? []) as $type):
              $selected = (int) $type->id === $idTypeValue ? 'selected' : '';
              ?>
              <option value="<?php echo (int) $type->id; ?>" <?php echo $selected; ?>
                data-libelle="<?php echo htmlspecialchars(strtolower((string) $type->libelle), ENT_QUOTES, 'UTF-8'); ?>">
                <?php echo htmlspecialchars((string) $type->libelle, ENT_QUOTES, 'UTF-8'); ?>
              </option>
            <?php endforeach; ?>
          </select>
          <small class="form-text text-muted">
            Sélectionnez "argent" si le besoin est en argent (pas de libellé ni prix requis)
          </small>
        </div>
      </div>
      ...
      <div class="mb-3" id="libelleContainer">
        <label class="form-label" for="libelle">
          <i class="bi bi-journal-text"></i> Libellé <span id="libelleRequired">*</span>
        </label>
      ...
      <div class="row">
        ...
        <div class="col-md-4 mb-3">
          <label class="form-label" for="prix_unitaire">
            <i class="bi bi-currency-dollar"></i> Prix unitaire (Ar) <span class="prix-required">*</span>
          </label>
          <input class="form-control" id="prix_unitaire" name="prix_unitaire" type="number" step="0.01" min="0" required
        ...
        <div class="col-md-4 mb-3">
          ...
          <input class="form-control" id="quantity" name="quantity" type="number" min="0" required
            value="<?php echo (int) $quantityValue; ?>" placeholder="0">
        </div>
        <div class="col-md-4 mb-3">
          <label class="form-label" for="quantity_restante">
            <i class="bi bi-boxes"></i> Quantité restante *
          </label>
          <input class="form-control" id="quantity_restante" name="quantity_restante" type="number" min="0" required
            value="<?php echo (int) $quantityRestanteValue; ?>" placeholder="0">
          <small class="form-text text-muted">
